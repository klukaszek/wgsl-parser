...
int parse_function_decl(char *shader_code, int *workgroup_sizes,
    ...
    for (unsigned long i = 0; i < sizeof(patterns) / sizeof(patterns[0]); i++) {
        match_length = slre_match(patterns[i], shader_code, strlen(shader_code),
                                  caps, 4, 0);
        if (match_length > 0) { // Match found
            ...
            int fn_index = i + 1; // Function name index depends on the number
                                  // of workgroup sizes
            memcpy(function_name, caps[fn_index].ptr, caps[fn_index].len);
            ...
            for (unsigned long j = 0; j <= i; j++) {
                if (caps[j].len > 0) {
                    char size_str[10];
                    memcpy(size_str, caps[j].ptr, caps[j].len);
